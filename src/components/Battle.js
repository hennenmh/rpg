...
class Battle extends Component {
  ...
  handleBattleClick = () => {
    if (this.isPlayerTurn) {
      ...
      this.roundNum++;
      this.characterPhysicalAttack(
        this.props.character.attack +
          (this.props.character.weapon.name !== 'Wand' &&
          this.props.character.weapon.name !== undefined
      ...
      this.enemyBaseAtk(
        this.props.enemy[this.enemyId].attack,
        this.props.character.defense +
          (this.props.character.armor.defense || 0),
        ...
        Math.floor(
          (this.props.character.attack +
            (this.props.character.weapon.name === 'Wand'
              ...
              : 0)) *
            1.5,
        ), // character does 1.5 times attack as magic
        this.props.enemy[this.enemyId].defense + Math.floor(this.enemyId * 0.5), // enemy has more magic defense as game goes on
        this.props.enemy[this.enemyId].health,
      );
      this.enemyBaseAtk(
        this.props.enemy[this.enemyId].attack,
  ...
  enemyCritAtk = (eAtk, cDef, cHealth, roll) => {
    ...
    this.enemyAtkText = `The ${
      this.props.enemy[this.enemyId].name
    } crit and did ${damage} damage!`;
  };
  ...
  checkResults = () => {
    const randomItemId = this.rollD6() - 1;
    if (
      this.props.enemy[this.enemyId].health <= 0 &&
      this.props.character.health > 0
    ) {
      if (this.enemyId === 4) {
        ...
        if (this.rollD100() < 20) {
          if (
            !this.props.character.inventory.includes(
              this.props.items[randomItemId],
            )
          ) {
  ...
  render() {
    const buttonText = `BATTLE! Round: ${this.roundNum}`;
    return (
      <div>
        <p>{buttonText}</p>
        ...
        <input
          type="button"
          value="Magic Attack"
          onClick={this.handleMagicClick}
        />
        <h4>Upcoming Monsters</h4>
        <ul>
          {this.props.enemy.map(
            list =>
              list.health > 0 && list.id !== this.enemyId ? (
                <img
                  key={list.id}
                  height="125px"
                  width="125px"
